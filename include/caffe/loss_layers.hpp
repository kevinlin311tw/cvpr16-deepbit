...
template <typename Dtype>
class EvenDistLossLayer : public LossLayer<Dtype> {
 public:
  explicit EvenDistLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EvenDistLoss"; }
  /**
...
template <typename Dtype>
class MinQuantizeLossLayer : public LossLayer<Dtype> {
 public:
  explicit MinQuantizeLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "MinQuantizeLoss"; }
  /**
...
template <typename Dtype>
class TransformLossLayer : public LossLayer<Dtype> {
 public:
  explicit TransformLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 4; }
  virtual inline const char* type() const { return "TransformLoss"; }
  /**
   * Unlike most loss layers, in the ContrastiveLossLayer we can backpropagate
   * to the first two inputs.
   */
